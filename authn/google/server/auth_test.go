...
import (
	...
	. "github.com/dpb587/ssoca/authn/google/server"
	"golang.org/x/oauth2"
	...
	Describe("OAuthUserProfileLoader", func() {
		Context("simple config", func() {
			...
			It("works", func() {
				...
				Expect(profile.Scopes).To(HaveLen(3))
				Expect(profile.Scopes).To(ContainElement("somebody@example.com"))
				Expect(profile.Scopes).To(ContainElement("email/mailbox/somebody"))
				Expect(profile.Scopes).To(ContainElement("email/domain/example.com"))
				Expect(profile.Attributes["name"]).To(Equal("test user"))
			})
		...
		Context("extended cloud project config", func() {
			...
			It("works", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				Expect(profile.Username).To(Equal("somebody@example.com"))
				Expect(profile.Scopes).To(HaveLen(5))
				Expect(profile.Scopes).To(ContainElement("somebody@example.com"))
				Expect(profile.Scopes).To(ContainElement("email/mailbox/somebody"))
				Expect(profile.Scopes).To(ContainElement("email/domain/example.com"))
				Expect(profile.Scopes).To(ContainElement("cloud/project/test-1234/roles/owner"))
				Expect(profile.Scopes).To(ContainElement("cloud/project/test-2345/roles/editor"))
				Expect(profile.Attributes["name"]).To(Equal("test user"))
			})
			...
			Context("filtering project/role", func() {
				...
				Context("filtering project", func() {
					...
					It("works", func() {
						...
						Expect(err).ToNot(HaveOccurred())
						Expect(profile.Username).To(Equal("somebody@example.com"))
						Expect(profile.Scopes).To(HaveLen(4))
						Expect(profile.Scopes).To(ContainElement("somebody@example.com"))
						Expect(profile.Scopes).To(ContainElement("email/mailbox/somebody"))
						Expect(profile.Scopes).To(ContainElement("email/domain/example.com"))
						Expect(profile.Scopes).To(ContainElement("cloud/project/test-1234/roles/owner"))
